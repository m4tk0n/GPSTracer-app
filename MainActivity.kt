...
class MainActivity : AppCompatActivity() {
    ...
    private fun checkForAppUpdate() {
        lifecycleScope.launch {
            val update = com.example.cycletracker.UpdateChecker.checkForUpdate() ?: return@launch
            if (update.versionCode > com.example.cycletracker.BuildConfig.VERSION_CODE) {
                androidx.appcompat.app.AlertDialog.Builder(this@MainActivity)
                    .setTitle("Nová verze k dispozici")
                    .setMessage("Verze ${update.versionName} je ke stažení." + (update.notes?.let { "\n\n$it" } ?: ""))
                    .setPositiveButton("Stáhnout") { _, _ ->
                        com.example.cycletracker.UpdateChecker.openDownload(this@MainActivity, update.apkUrl)
                    }
    ...
    private fun showSettingsMenu(anchor: android.view.View) {
        ...
        popup.menu.add("Trasy").setOnMenuItemClickListener {
            startActivity(Intent(this, TrackListActivity::class.java))
            true
        }
        popup.menu.add("Offline mapy").setOnMenuItemClickListener {
